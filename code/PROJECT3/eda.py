...
# 각 컬럼에 대해 히스토그램 그리기
for i, column in enumerate(df_pic.columns):
    upper_97_percentile = df_pic[column].quantile(0.97)
    ...
    # 해당 서브플롯에 히스토그램 그리기
    sns.histplot(data=df_pic, x=column, binrange=[0, upper_97_percentile], ax=axes[row, col])
    
    # 제목, 라벨 설정
    axes[row, col].set_title(f'Histogram of {column} (97% percentile)')
...
# ----------------------------------------
## 연진 수정 (연속형 boxplot, kdeplot subplot 4x7로 변경)
import matplotlib.pyplot as plt
import seaborn as sns

# 서브플롯 설정: 4x7 (총 28개의 공간 생성)
fig, axes = plt.subplots(4, 7, figsize=(35, 20))
fig.tight_layout(pad=5.0)

# 연속형 변수 개수에 맞게 서브플롯 그리기
for i, col in enumerate(continuous_cols):
    # 현재 플롯 위치 계산 (i번째 연속형 변수를 배치할 곳)
    row_box = (i // 7) * 2      # Boxplot이 있는 행 (짝수 행)
    col_box = i % 7             # Boxplot의 열

    row_kde = row_box + 1       # KDE Plot은 Boxplot 바로 아래 (홀수 행)
    col_kde = col_box

    # Box Plot
    sns.boxplot(x='target', y=col, hue='target', data=df, ax=axes[row_box][col_box])
    axes[row_box][col_box].set_title(f'Box Plot of {col} by Target')
    axes[row_box][col_box].legend().set_visible(False)  # 중복된 범례 제거

    # KDE Plot
    sns.kdeplot(data=df[df['target'] == 0][col], label='target=0', ax=axes[row_kde][col_kde], fill=True)
    sns.kdeplot(data=df[df['target'] == 1][col], label='target=1', ax=axes[row_kde][col_kde], fill=True)
    axes[row_kde][col_kde].set_title(f'KDE Plot of {col} by Target')

# 플롯이 없는 빈 공간 제거
for j in range(i+1, 14):  # 14는 4x7 그리드 내 플롯 수 (28 / 2) 
    fig.delaxes(axes[(j // 7) * 2][j % 7])     # Boxplot 영역
    fig.delaxes(axes[(j // 7) * 2 + 1][j % 7]) # KDE 영역
...
plt.show()
# --------------------------------------
## 연진 수정 3x6 서브플롯 설정 (18칸)
fig, axes = plt.subplots(3, 6, figsize=(24, 15))

# 각 서브플롯에 scatterplot 추가
for i, col in enumerate(df.columns[:-1]):
    row = i // 6  # 행 인덱스 계산
    col_idx = i % 6  # 열 인덱스 계산
    sns.scatterplot(data=df, x=col, y='target', hue='target', ax=axes[row, col_idx])
    axes[row, col_idx].set_title(f'Scatterplot of {col}')
...
total_columns = len(df.columns[:-1])
if total_columns < 18:
    for j in range(total_columns, 18):
        axes[j // 6, j % 6].set_visible(False)  # 남는 서브플롯을 비활성화
...
for i, (x_col, y_col) in enumerate(scatter_pairs):
    row = i // 3  # 행 인덱스 계산
    col = i % 3   # 열 인덱스 계산
    sns.scatterplot(x=x_col, y=y_col, data=df, ax=axes[row, col])
    axes[row, col].set_title(f'Scatter Plot of {x_col} vs {y_col}')

# 빈 서브플롯 처리 (만약 scatter_pairs에 6개 이하의 쌍이 있다면)
if len(scatter_pairs) < 6:
    for j in range(len(scatter_pairs), 6):
        fig.delaxes(axes[j // 3, j % 3])  # 빈 서브플롯 삭제
...
# 빈 서브플롯 처리 (만약 scatter_pairs_pearson에 6개 이하의 쌍이 있다면)
if len(scatter_pairs_pearson) < 6:
    for j in range(len(scatter_pairs_pearson), 6):
        fig.delaxes(axes[j // 3, j % 3])  # 빈 서브플롯 삭제
...
plt.title('"unknown1" 변수에 따른 target 값의 분포')
plt.xlabel('Type')
plt.ylabel('Count')
plt.legend(title='Target', labels=['정상 (0)', '이상 (1)'])
